...
class Hamiltonian(Matrix):
    ...
    def H0(self, capacity, at_count, n_levels, wc, wa, g):
        H_dim = len(self.states)
        # H_dim = (capacity['0_1']+1) * (capacity['1_2']+1) * \
        #     pow(n_levels, at_count)

        # H0 = bsr_matrix((H_dim, H_dim))
        H0 = lil_matrix((H_dim, H_dim))
        
        
        for i in range(len(self.states)):
            # print(self.states[i]['ph'])
            cnt = 0
        ...
        return H0
    # def H0_c(self, capacity, at_count, n_levels, wc, wa, g):
    #     adiag = {}
    #     across = {}
    #     a = {}
    #     acrossa = {}
        ...
        for i in range(len(self.states)):
            for k in self.cavity.wa.keys():
                for at in range(len(self.cavity.wa[k])):
                    H0[i, i] += self.cavity.wa[k][at] * self.states[i]['at'][at]
        
        for i in H0.toarray():
            print(i)
    
    def H1c(self, capacity, at_count, n_levels, wc, wa, g):
        # -----------------------------------------
        ...
        for i in range(len(self.states)):
            ...
            for j in range(len(self.states)):
                ...
                for at in range(self.cavity.n_atoms):
                    ...
                    if self.states[i]['at'][at] == self.states[j]['at'][at]:
                        continue

                    if abs(self.states[i]['at'][at] - self.states[j]['at'][at]) == 1:
                        diff_atoms_type = max(self.states[i]['at'][at], self.states[j]['at'][at])

                        diff_at_ji = self.states[j]['at'][at] - self.states[i]['at'][at]
                        diff_atoms+=1
                        diff_at_pos = at
                ...
                if diff_atoms_type == diff_ph_pos and diff_ph_ji + diff_at_ji == 0:
                    H0[i, j] = H0[j, i] = self.cavity.g['0_1'][diff_at_pos] * sqrt(max(ph_i[ph], ph_j[ph]))
    ...
    def HI_RWAc(self, capacity, at_count, n_levels, wc, wa, g):
        adiag = {}
        across = {}
        a = {}
        acrossa = {}
